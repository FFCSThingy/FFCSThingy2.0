...
class TimetableCell extends Component {
	renderBreak = () => <td width="8px" rowSpan={9} className="timetableBreak"><strong>{this.props.children}</strong></td>

	renderTime = () => {
		const cellClass = (this.props.lab) ? 'timetableLabHours' : 'timetableTheoryHours';
		return <td key={this.props.children} className={cellClass}>{this.props.children}</td>;
	}

	renderDay = () => <td key={this.props.children} className="timetableDay">{this.props.children}</td>

	renderEmpty = () => <td key={this.props.children} className="timetableEmpty"><b>{this.props.children}</b></td>

	renderFilled = () => {
		const cellClass = (this.props.lab) ? 'timetableFilledLab' : 'timetableFilledTheory';
		return (
			<td key={this.props.children} className={cellClass}>
					...
					overlay={(
						<Tooltip>
							<h6 className="courseDetails">
								<b>{this.props.reqdCourse.title}</b>
								<br />
								{this.props.reqdCourse.faculty}
								<br />
				...
				>
					<div>
						...
						<h6 className="courseDetails">
							{`${this.props.reqdCourse.venue} - ${this.props.reqdCourse.course_type}`}
						</h6>
					</div>
				</OverlayTrigger>
			</td>
